...
async function main() {
  console.log('Testing sendMessage with direction BUSINESS_TO_PARTICIPANT and NO sender...');
  try {
    const participantId = { anonymousVisitorId: '00000000-0000-0000-0000-000000000007' };
    ...
    const messagePayload = {
      ...
      content: {
        basic: {
          items: [
            {
              text: 'Dette er et test-svar fra boten/business uten sender.'
            }
          ]
        }
      }
    ...
    const res = await wixClient.inboxMessages.sendMessage(convId, messagePayload);
    console.log('Success sending business message! Response:', JSON.stringify(res, null, 2));
  } catch (err) {
    console.error('Failed sending business message!');
