...
async function main() {
  ...
  for (const order of items) {
    ...
    for (const item of lineItems) {
      ...
      const customLines = descLines.filter(line => line.modifierDescriptionLine === true);
      if (customLines.length > 0) {
        console.log(`\nFound modifierDescriptionLine in Order ID: ${order.id}, Number: ${order.number}`);
  ...
  if (!found) {
    console.log('No past orders with modifierDescriptionLine = true were found.');
  }
}
