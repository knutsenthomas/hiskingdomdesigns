...
async function main() {
  const checkoutId = 'ca727402-9a4b-4d7f-81ec-5fff6f1e996a';
  console.log('Inspecting checkout via API key:', checkoutId);
  try {
    const res = await wixClient.checkout.getCheckout(checkoutId);
    console.log('Checkout details:', JSON.stringify(res, null, 2));
  } catch (err) {
    console.error('Failed to inspect checkout:', err.message);
  }
}
