  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      query: {
        filter: {
          $or: [
            { 'info.name.first': { $startsWith: 'Thomas' } },
            { 'info.name.last': { $startsWith: 'Knutsen' } },
            { 'primaryInfo.email': { $startsWith: 'thomas' } },
            { 'primaryInfo.email': { $startsWith: 'knutsenthomas' } }
          ]
        }
      }
    })
