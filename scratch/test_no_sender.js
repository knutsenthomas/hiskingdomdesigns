...
async function main() {
  console.log('Testing sendMessage with direction PARTICIPANT_TO_BUSINESS but NO sender...');
  try {
    const participantId = { anonymousVisitorId: '00000000-0000-0000-0000-000000000006' };
    ...
    const messagePayload = {
      ...
      content: {
        basic: {
          items: [
            {
              text: 'Dette er en testmelding uten sender.'
            }
          ]
        }
      }
    ...
    console.log('Success without sender! Response:', JSON.stringify(res, null, 2));
  } catch (err) {
    console.error('Failed without sender!');
